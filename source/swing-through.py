...
def printClearance(ShipP1X,ShipP1Y,ShipP2X,ShipP2Y,CpP1X,CpP1Y,CpP2X,CpP2Y,directory,figNo, title):
    plt.figure(figNo,figsize=(25,12))
    plt.suptitle(title,fontsize=14,fontweight='bold')

    pltTimeHistoryForce(ShipP1X, 221, 'Time [sec]', 'Position [m]', 'X-Positions')
    pltTimeHistoryForce(CpP1X, 221, 'Time [sec]', 'Position [m]', 'X-Positions')
    pltTimeHistoryForce(ShipP2X, 221, 'Time [sec]', 'Position [m]', 'X-Positions')
    pltTimeHistoryForce(CpP2X, 221, 'Time [sec]', 'Position [m]', 'X-Positions')

    pltTimeHistoryForce(ShipP1X, 222, 'Time [sec]', 'Position [m]', 'Y-Positions')
    pltTimeHistoryForce(CpP1X, 222, 'Time [sec]', 'Position [m]', 'Y-Positions')
    pltTimeHistoryForce(ShipP2X, 222, 'Time [sec]', 'Position [m]', 'Y-Positions')
    pltTimeHistoryForce(CpP2X, 222, 'Time [sec]', 'Position [m]', 'Y-Positions')

    d1 = []
    d2 = []
    
    for i in range(0,len(ShipP1X)):
        d1.append(getSign(ShipP1X[i] - CpP1X[i])*sqrt((ShipP1X[i] - CpP1X[i])**2 + (ShipP1Y[i] - CpP1Y[i])**2))
        d2.append(getSign(CpP2X[i] - ShipP2X[i])*sqrt((ShipP2X[i] - CpP2X[i])**2 + (ShipP2Y[i] - CpP2Y[i])**2))
        
    pltTimeHistoryForce(d1, 223, 'Time [sec]', 'Distance [m]', 'Clearance FWD Time History')
    pltTimeHistoryForce(d2, 224, 'Time [sec]', 'Distance [m]', 'Clearance AFT Time History')

    plt.tight_layout()
    plt.subplots_adjust(top=0.92)
    plt.savefig(directory + '/14_clearance.png',bbox_inches='tight')
...
directory = "E:/STORAGE/ANSYS PROJECTS/H5_0117_Husky_R1/results_swingthrough/Tp11"
...
ship.p1X.append(readCSV(directory + "/AQWA046.CSV")) # X-coordinate of Crane Point 1 in FRA
ship.p1Y.append(readCSV(directory + "/AQWA047.CSV")) # Y-coordinate of Crane Point 1 in FRA
ship.p2X.append(readCSV(directory + "/AQWA048.CSV")) # X-coordinate of Crane Point 2 in FRA
ship.p2Y.append(readCSV(directory + "/AQWA049.CSV")) # Y-coordinate of Crane Point 2 in FRA
...
cPlate.p1X.append(readCSV(directory + "/AQWA050.CSV")) # X-coordinate of Crane Point 1 in FRA
cPlate.p1Y.append(readCSV(directory + "/AQWA051.CSV")) # Y-coordinate of Crane Point 1 in FRA
cPlate.p2X.append(readCSV(directory + "/AQWA052.CSV")) # X-coordinate of Crane Point 2 in FRA
cPlate.p2Y.append(readCSV(directory + "/AQWA053.CSV")) # Y-coordinate of Crane Point 2 in FRA
...
printClearance(ship.p1X[0],ship.p1Y[0],ship.p2X[0],ship.p1Y[0],cPlate.p1X[0],cPlate.p1Y[0],cPlate.p2X[0],cPlate.p1Y[0],directory,14,"Clearances between the Cranes and the Cover Plate Time History")